...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    map<std::string, std::vector<std::string>>::const_iterator lookup = dict.find(newword1);
    map<std::string, std::vector<std::string>>::const_iterator lookup2 = dict.find(newword2);
    if(lookup == dict.end() || lookup2 == dict.end()){
      return false;
    }
    if(lookup->second.size() != lookup2->second.size()){
      return false;
    }
    else{
      for(size_t i = 0; i < lookup->second.size(); i++){
        if(lookup->second[i] != lookup2->second[i]){
          return false;
        }
      }
    }
